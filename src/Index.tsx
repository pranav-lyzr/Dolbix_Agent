...
const Index = () => {
  ...
  const handleProcess = async () => {
    if (kitoneFile && zacFile) {
      ...
      console.log("Navigating to /report", kitoneData, zacData, dataCodeData);
      // Navigate to /report and pass the processed data as state
      navigate('/report', { state: { kitoneData, zacData, dataCodeData } });
    }
  };
  ...
  const processZacData = (data: any[]) => {
    return data
      .map((row) => {
        ...
        if (salespersonCode.includes('(Subtotal)') || salespersonCode.includes('(total)') || salespersonCode.includes('（小計）') || salespersonCode.includes('(合計)')) {
          return null;
        }
    ...
    <div className="min-h-screen bg-gradient-to-br from-white-50 to-purple-100">
      <div className="container mx-auto px-4 py-16">
        <div className="max-w-4xl mx-auto">
          ...
          <p className="text-center mb-12 text-gray-600">
            Upload your Kitone and ZAC (ERP) sheets for processing. DataCode Mapping is optional.
          </p>
          ...
          <div className="grid md:grid-cols-2 gap-8 mb-12">
            ...
            <FileUpload label="DataCode Mapping Sheet (Optional)" file={dataCodeFile} onChange={setDataCodeFile} />
          </div>

          <ProcessButton onClick={handleProcess} disabled={!kitoneFile || !zacFile} />
        </div>
      </div>
    </div>
  );
